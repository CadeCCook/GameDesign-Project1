...
if (global.show_shop) {
    ...
    // control upgrade
    var cy  = py + 230;
    var mbx1 = px+16, mby1 = cy, mbx2 = px+pw-16, mby2 = cy+52;
    ...
    var vy   = py + 290;
    var vbx1 = px+16, vby1 = vy, vbx2 = px+pw-16, vby2 = vy+52;

    var coin_lvl  = global.shop.coin_mult_level;
    var coin_max  = global.shop.coin_mult_max;
    var coin_cost = 500 * power(2, coin_lvl);
    var can_buy_val = (coin_lvl < coin_max) && (global.coins >= coin_cost);

    draw_set_colour(can_buy_val ? make_colour_rgb(240, 180, 40) : make_colour_rgb(90,90,90));
    draw_rectangle(vbx1, vby1, vbx2, vby2, false);

    draw_set_colour(c_white);
    var coin_label = "Coin Value (x2 per level)   Lvl " + string(coin_lvl) + "/" + string(coin_max) +
                     "   Current: x" + string(power(2, coin_lvl));
    draw_text(vbx1+12, vby1+8, coin_label);

    var coin_price = (coin_lvl < coin_max) ? ("Cost: " + string(coin_cost) + " coins") : "MAXED";
    draw_text(vbx1+12, vby1+28, coin_price);

    if (mouse_check_button_pressed(mb_left)) {
        var mx = device_mouse_x_to_gui(0);
        var my = device_mouse_y_to_gui(0);
        if (point_in_rectangle(mx, my, vbx1, vby1, vbx2, vby2)) {
